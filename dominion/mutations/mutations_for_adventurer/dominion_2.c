...
void _cardEffect_village(int currentPlayer, struct gameState* state, int handPos){
	//+1 Card
      drawCard(currentPlayer, state);
...
void _cardEffect_minion (int currentPlayer, struct gameState*  state, int handPos, int choice1, int choice2){
	//+1 action
	int i;
	int j;
      state->numActions++;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
      ...
      /* XXX: original code was : 	state->discard[currentPlayer][state->discardCount[currentPlayer]++]=temphand[z-1]; // discard all cards in play that have been drawn */
	state->discard[ 1 + currentPlayer][state->discardCount[currentPlayer]++]=temphand[z-1]; // discard all cards in play that have been drawn
	z=z-1;
      }
      return 0;
      ...
      for (i = 0; i < state->numPlayers; i++)
	{
      ...
      discardCard(handPos, currentPlayer, state, 0);
	*/
	_cardEffect_councilroom(currentPlayer, state, handPos);
      return 0;
	...
	/*
      ...
      //discard played card from hand
      discardCard(handPos, currentPlayer, state, 0);*/
	  _cardEffect_village(currentPlayer, state, handPos);
      return 0;
	...
	/*
      ...
      //discard card from hand
      discardCard(handPos, currentPlayer, state, 0);*/
	  _cardEffect_great_hall(currentPlayer, state, handPos);
      return 0;
	...
	_cardEffect_minion(currentPlayer, state, handPos, choice1, choice2);
      return 0;
